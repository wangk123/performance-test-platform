...
> 自 2026-08-28 起，功能大纲与架构目标统一由 `docs/architecture-and-roadmap.md` 管理（含目标架构图、分级清单、验收口径与决策记录），以下为同步摘录，以该文档为准。

| 优先级 | 工作项 |
|--------|--------|
| P0 | 计划文档模块重构（结构化字段+Markdown、状态机与评审、模板体系、自动回填、冲突处理） |
| P0 | MCP 计划工具集 + 仓库内 `skills/perf-plan/` 本地生成 skill |
| P0 | 验收标准 + 报告自动判等（指标可选、文档内解析，无指标判等跳过） |
| P0 | MySQL 一次性迁移（主库 + 执行明细；H2 留本地开发） |
| P1 | 环境检查（SSH 探测、插件式检查项、分级自动修复、三态清单） |
| P1 | 缺陷台账 + Jira 同步（可插拔 gateway） |
| P1 | 通知（MCP 邮件：模板 + 抄送清单，项目/计划两级） |
| P1 | 迭代对比（发布版本快照 + 手动基线） |
| P1 | CSV 测试数据上传与节点分发 |
| P2 | 部署整理（后端托管前端单 jar + 启动脚本 + 部署文档） |
| P2 | 容量调度/自动分片（远期） |
| P3 | 多团队空间/RBAC、AI 生成脚本、CI、SSO 预留 |

> 说明：旧表中已实现项（辅助脚本、Git AI 等）与未排期甜点项（多引擎导出、.jmx 下载、Word 导出增强、告警、造数导出、Groovy 函数调试）不再单列，详见 `docs/architecture-and-roadmap.md` §6。
