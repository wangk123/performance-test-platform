...
- [ ] 定义统一游标分页协议（条数/字节/耗时预算），复用 T4 封套字段
- [ ] 聚合/秒级指标查询接入游标 + 预算
- [ ] 失败样本（SQLite）查询接入游标 + 预算（复用现有 cursor 雏形）
- [ ] Prometheus 指标查询接入时间窗 + 粒度 + 预算
- [ ] 定义数据可用性返回：存在性/时间覆盖/采样/截断/可回溯/缺失原因
- [ ] 约束：禁止用其他时段/实例/数据源静默替代缺失数据
- [ ] 先失败测试：超预算→截断+游标；缺失源→显式缺失而非空成功
- [ ] 验证：`./gradlew :backend:test` 有界查询与可用性用例通过
...
- [ ] 定义统一关联键：`executionId` + 绝对时间窗 + 目标实例(host/instance/service) + 请求标签 + 可选 `traceId`
- [ ] 时钟对齐：压测端 / 被测端 / Prometheus 三方时间基准校正策略
- [ ] 新建 `evidence` 适配层：将现有执行/指标/样本/Prometheus 统一为带关联键的证据摘要
- [ ] 接入约束：新数据源必须声明其绑定的关联键，否则不接入
- [ ] 数据保留/失效：执行删除→证据失效提示；只存摘要+来源定位
- [ ] 先失败测试：跨源按 executionId+时间窗对齐正确；时钟偏移被校正
- [ ] 验证：`./gradlew :backend:test` 关联用例通过
...
- [ ] 新建 `analysis` 模块，算法带版本号
- [ ] 趋势分析：响应时间/吞吐/错误率变化
- [ ] 异常区间 / 性能拐点检测
- [ ] 错误聚类 + 请求/接口贡献度
- [ ] 资源饱和（CPU/内存/GC/线程/连接池）+ 压测与资源指标时间相关性
- [ ] 执行间可比性与差异（基线 vs 候选）
- [ ] 输出结构：事实 + 算法版本 + 输入范围 + 完整度 + 证据定位（不含根因）
- [ ] 建固定「黄金数据集」+ 可重复性测试
- [ ] 验证：`./gradlew :backend:test` 黄金样本重复运行结果一致
...
- [ ] 写操作（创建/启动）支持客户端幂等键，重试返回原结果不重复启动
- [ ] 执行预检 + 影响评估接口
- [ ] 启动/停止/取消统一语义
- [ ] 异步任务返回稳定任务 ID + 状态查询工具（不依赖长同步连接）
- [ ] 先失败测试：同一幂等键重复请求只启动一次；取消后状态一致
- [ ] 验证：`./gradlew :backend:test` 幂等与异步状态用例通过
