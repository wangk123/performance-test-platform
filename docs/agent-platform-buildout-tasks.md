...
- 涉及：`execution` `facade` `verification`(新)。依赖：T7 T8。
- 验收：取证需审批；验证能输出「改善/退化/无法判定」。
- 实现：`verification` 包——取证（`evidence_captures`：目的/影响/成本预检 → 人工审批（仅 HUMAN）→ 执行有界证据快照，bundle 定位回流）+ 变更登记（`change_records`）+ 优化验证（`verification_records`：复用 T7 execution-compare + 错误率护栏，三态结论 IMPROVED/REGRESSED/INCONCLUSIVE），经 `VerificationFacade` + agent 面入口暴露。

- [x] 取证动作预检（目的/影响/成本）+ 审批 + 执行 + 新数据回流
- [x] 变更登记：代码/配置引用登记入口
- [x] 验证：基线 vs 候选可比性检查 + 护栏指标
- [x] 验证结论三态输出：改善 / 退化 / 无法判定（波动过大→无法判定）
- [x] 语义分离校验：取证与验证不混为同一流程
- [x] 先失败测试：未审批取证被拒；不可比执行→无法判定
- [x] 验证：`./gradlew :backend:test` 取证/验证用例通过

### T10 治理（脱敏 / 审计 / 限流） —— [✅ 完成]
- 目标：横切治理，进模型/出平台前强制生效。
- 涉及：跨模块（挂在 Facade / 鉴权层）。依赖：T3。
- 验收：敏感字段在输出前脱敏；请求/执行可审计重建；超配额被限流。
- 实现：`governance` 包——`RedactionService`（敏感键/令牌/头部脱敏）、`AgentGovernanceFilter`（仅 `/api/agent/**`：滑动窗口速率限流 + 在途并发限流 + 请求审计 + 输出边界脱敏）、`request_audit`/`execution_audit` 落库，`ExecutionFacade` 在 START/STOP/CANCEL 时写执行审计；配置 `platform.governance.*`。

- [x] 脱敏：请求体/响应体/SQL 参数/日志/环境，在 Facade 输出前强制（不依赖 SKILL）
- [x] 审计：请求审计 + 执行审计，可重建平台侧操作轨迹
- [x] 限流：并发/超时/配额/速率限制
- [x] 先失败测试：敏感字段未脱敏则测试失败；超速率→429
- [x] 验证：`./gradlew :backend:test` 治理用例通过
