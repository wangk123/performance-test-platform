...
| D3 | AI 生成计划 | 平台提供 **MCP 接口**（模板/创建/读取/更新/查询）+ 模板（内置通用 Markdown 占位符）；**本地 Agent 通过仓库内 `skill-pack/perf-platform-plan/` 梳理需求、生成、同步**；平台内与本地可交替编辑。（2026-09-07 修订：模板只保留**内置预设**、随平台版本统一演进——新模板用于新建，解析契约对存量文档向后兼容，不兼容变更须附迁移；项目自定义模板不列入范围；结构化/MD 双主存储与运行时双向同步经分析明确不做，MD 文档保持唯一事实源，映射确定性由结构契约保证） |
...
| D6 | 验收判等 | 验收标准实体挂在计划上（目标 TPS/P95/P99/错误率/并发峰值/容量），报告自动判定 + 人工确认。（2026-09-07 修订：验收标准**可选**——无指标计划〔摸底/排查型〕判等跳过、结论纯人工；判等输入 = 文档指标章节〔保存时严格校验、报告生成时确定性解析，**不建判等实体**〕；场景级 + 交易级（含 P99）两层判定，行状态 达成/未达成/无法判定，见 P0-3 设计） |
| D7 | 环境检查 | **独立预检任务**，平台本机优先，**SSH 连目标服务器探测**；SSH 做不了的 push 脚本到目标机执行；结果三态：本身 OK / 有问题已修复 / 需手动修复（附建议与方法）。（2026-09-02 修订：环境检查是**测试前的执行动作**——挂计划执行设置（非文档内容、不进评审），勾选启用 + 配置检测清单，评审通过后**首次执行时自动运行**（挂现有执行预检 seam），可手动跳过；探测/修复能力仍由 P1-1 交付。2026-09-15 修订〔P1-1 设计〕：目标机 = 文档「环境部署信息」表 + 项目级 SSH 凭据池（计划可覆盖）；人工确认项移出检查清单；回填只做系统批注 + 测试资源章节摘要行，不联动入口准则；见 P1-1 设计） |
| D8 | 自动修复边界 | 三级：**低风险默认自动修**（连接池/超时等应用配置）/ 中风险需勾选（ulimit/内核参数）/ 只建议（DB/中间件）；所有修复留 diff 可回滚，全局开关；**检查项插件式扩展，平台默认只做通用低风险排查，项目特性自行扩展自担风险**（2026-09-15 修订〔P1-1 设计〕：**取消自动修复**——全部改为用户确认后批量修复（低风险默认勾选 / 中风险手动勾选 / 高风险勾选单独确认），风险等级仅决定提示强度；检查项 = 代码级注册表（Spring Bean），新增检测项页面自动出现；全局开关改为"允许平台执行修复"总闸；见 P1-1 设计） |
...
| D14 | 数据层 | **MySQL 为运行时唯一数据源**（服务器 Docker 部署，本地调试默认连服务器，不做 H2 运行时兜底）；**执行明细（失败样本 SQLite/jsonl、jmeter 日志、结果文件）不迁库**，维持平台宿主机按执行分文件；schema 走 Flyway（V1 基线 + 增量）；存量 H2 数据由一次性迁移工具带入；H2 仅作为测试内存库。（2026-09-07 修订：原口径「主库 + 执行明细统一迁 MySQL；H2 留本地开发 profile」——明细体量大〔失败率高时单次执行 GB 级、LONGTEXT 重〕、按执行分文件删除即删净是运维优势、且近中期无跨执行查样本需求，收窄为主库迁移；详见 P0-4 设计） |
...
flowchart TB
    subgraph ACCESS["接入层"]
        ...
        AGENT["本地 Agent<br/>Claude Code / DSH · skill-pack/perf-platform-plan"]
    end
...
| `skill-pack/perf-platform-plan/` | 本地 Agent 生成压测计划用的 skill（对话梳理 → 拉模板 → 生成 → MCP 同步） | 平台 MCP 工具 | 随仓库分发 |
...
> 状态：⬜ 未开始｜🔄 进行中｜🟨 代码完成、待人工验收｜✅ 已完成。**实施顺序由 P0 到 P3；同优先级内按"闭环断点优先"。**
...
| ID | 工作项 | 状态 | 依赖 | 完成口径（验收） |
|----|--------|------|------|------------------|
| P1-1 | **环境检查**：独立预检任务（平台本机 SSH 探测，push 脚本兜底）；插件式检查项；三级自动修复边界 D8；三态结果清单 + 修复建议；结果回填计划文档 | 🟨（代码完成、待人工验收；18 commits 于 feature/p1-1-env-check，设计见 `docs/superpowers/specs/2026-09-15-p1-1-env-check-design.md`；终审遗留优化项 P1-2 候选：事务外探测、409 body 带 failures、修复记录真 diff、凭据唯一索引、MethodExecTable 弹窗对齐） | P0-1 | 对一台"配置有问题"的服务器跑预检，输出三态清单；低风险项可一键修复且留 diff |
...
| P1-5 | **CSV 测试数据管理**：参数文件上传、版本管理、执行时分发到各节点；与脚本 CSV 步骤联动 | 🟨（代码完成、待人工验收；22 commits 于 feature/p1-5-p1-7-script-module，设计见 `docs/superpowers/specs/2026-09-16-p1-5-p1-7-script-module-design.md` §3：项目级数据文件资产 + 场景级绑定〔计划文档中关联脚本后下拉选择〕+ worker 全量依赖分发修复；2026-09-16 本机 18080 新构建实例冒烟通过：CSV 上传 201〔sha256/表头/行数解析〕、列表/版本接口、删除 204 且列表回空；待人工验收：多节点执行 controller/worker 均拿到文件、variableNames 出现在请求参数、同名 fileName 冲突报错含步骤名、换绑脚本版本后失效绑定被剔除） | — | 上传一个 CSV，配置到场景，多节点执行时每个节点都拿到该文件 |
| P1-7 | **JSR223 前后置处理器组件**：编辑器支持给线程组/请求添加 JSR223 PreProcessor / PostProcessor（Groovy 优先）；内置加解密代码片段库（AES/签名等）；密钥走参数化引用不落明文（D17）；渲染进 JMX 且单机/分布式执行均生效 | 🟨（代码完成、待人工验收；22 commits 于 feature/p1-5-p1-7-script-module，设计见 `docs/superpowers/specs/2026-09-16-p1-5-p1-7-script-module-design.md` §4，含 `${__P()}` 属性透传缺口修复〔场景 jmeterProperties → payload → -J/-G〕与明文密钥扫描警告；2026-09-16 本机 18080 新构建实例冒烟：`GET /api/jsr223-snippets` 返回 6 条片段；待人工验收：分布式执行签名/属性双端〔controller -J / worker -G〕生效验证、导出 JMX 在本地 JMeter GUI 打开元素完整且 `-JsignKey=xx` 本地跑通） | — | 编辑器添加带请求签名的 JSR223 PreProcessor，导出 JMX 在 JMeter 与分布式执行中均生效 |
...
> P1-2（缺陷台账）、P1-3（通知）已于 2026-09-02 延期至 P3（P3-7 / P3-8）。P1-6（MCP 工具目录页）已于 2026-09-04 并入 P0-2。
...
- **项目自定义模板**（2026-09-07）：模板只保留**内置预设**、随平台版本统一演进（新模板用于新建；解析契约对存量文档向后兼容，不兼容变更须附迁移）。P0-1 已交付的项目级模板创建接缝（`POST /projects/{id}/plan-templates`）随 P0-3 实施或 P0-1 验收走查决定下线或保留。**结构化/MD 双主存储、运行时双向同步、AI 影子数据修正明确不做**——预设模板下映射由结构契约保证确定性（要么成功、要么显式失败，不静默错），MD 文档保持唯一事实源；AI 辅助若将来引入，仅限入口归一化（人确认后写回文档）。
...
2. **第二批**：P0-2（MCP 计划工具 + skill + 工具目录页）、P0-3 判等——闭环两端接通（目录页部分无阻塞，可提前穿插）。
3. **第三批起**：P1 按 P1-1 → P1-4 → P1-5 顺序推进（每项独立可交付）；P1-7（JSR223）无阻塞依赖，可与第三批并行穿插；P1-8（监控深化）是 P2-4 告警的前置，按序推进。P1-2（缺陷台账）、P1-3（通知）已延期至 P3（P3-7 / P3-8）：P2-4 的邮件通知依赖 P3-8，告警展示与熔断可先做。
