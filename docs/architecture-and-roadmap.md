...
| P1-5 | **CSV 测试数据管理**：参数文件上传、版本管理、执行时分发到各节点；与脚本 CSV 步骤联动 | 🟨（代码完成、待人工验收；23+ commits 于 feature/p1-5-p1-7-script-module，设计见 `docs/superpowers/specs/2026-09-16-p1-5-p1-7-script-module-design.md` §3：项目级数据文件资产 + 场景级绑定〔计划文档中关联脚本后下拉选择〕+ worker 全量依赖分发修复；2026-09-16 本机 18080 新构建实例冒烟通过：CSV 上传 201〔sha256/表头/行数解析〕、列表/版本接口、删除 204 且列表回空；待人工验收：多节点执行 controller/worker 均拿到文件、variableNames 出现在请求参数、同名 fileName 冲突报错含步骤名、换绑脚本版本后失效绑定被剔除） | — | 上传一个 CSV，配置到场景，多节点执行时每个节点都拿到该文件 |
| P1-7 | **JSR223 前后置处理器组件**：编辑器支持给线程组/请求添加 JSR223 PreProcessor / PostProcessor（Groovy 优先）；内置加解密代码片段库（AES/签名等）；密钥走参数化引用不落明文（D17）；渲染进 JMX 且单机/分布式执行均生效 | 🟨（代码完成、待人工验收；23+ commits 于 feature/p1-5-p1-7-script-module，设计见 `docs/superpowers/specs/2026-09-16-p1-5-p1-7-script-module-design.md` §4，含 `${__P()}` 属性透传缺口修复〔场景 jmeterProperties → payload → -J/-G〕与明文密钥扫描警告；2026-09-16 本机 18080 新构建实例冒烟：`GET /api/jsr223-snippets` 返回 6 条片段；待人工验收：分布式执行签名/属性双端〔controller -J / worker -G〕生效验证、导出 JMX 在本地 JMeter GUI 打开元素完整且 `-JsignKey=xx` 本地跑通） | — | 编辑器添加带请求签名的 JSR223 PreProcessor，导出 JMX 在 JMeter 与分布式执行中均生效 |
