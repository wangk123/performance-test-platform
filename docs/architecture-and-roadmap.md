...
| P1-5 | **CSV 测试数据管理**：参数文件上传、版本管理、执行时分发到各节点；与脚本 CSV 步骤联动 | ⬜（设计见 `docs/superpowers/specs/2026-09-16-p1-5-p1-7-script-module-design.md` §3：项目级数据文件资产 + 场景级绑定〔计划文档中关联脚本后下拉选择〕+ worker 全量依赖分发修复） | — | 上传一个 CSV，配置到场景，多节点执行时每个节点都拿到该文件 |
| P1-7 | **JSR223 前后置处理器组件**：编辑器支持给线程组/请求添加 JSR223 PreProcessor / PostProcessor（Groovy 优先）；内置加解密代码片段库（AES/签名等）；密钥走参数化引用不落明文（D17）；渲染进 JMX 且单机/分布式执行均生效 | ⬜（设计见 `docs/superpowers/specs/2026-09-16-p1-5-p1-7-script-module-design.md` §4，含 `${__P()}` 属性透传缺口修复〔场景 jmeterProperties → payload → -J/-G〕与明文密钥扫描警告） | — | 编辑器添加带请求签名的 JSR223 PreProcessor，导出 JMX 在 JMeter 与分布式执行中均生效 |
