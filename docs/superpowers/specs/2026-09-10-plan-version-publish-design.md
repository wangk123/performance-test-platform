...
## 9. 修订记录合并——发布动作登记为版本（2026-09-11 增补）

背景：「发布」Tab 的发布快照历史与「版本」Tab 的修订记录是同一类事实的两套账。合并为**一份修订记录**，`plan_versions` 成为计划从创建到报告发布全程的唯一版本史。

- **来源标记**：`plan_versions` 增 `kind` 列（`MANUAL` 手动发版 / `PUBLISH` 报告发布），Flyway V6 增列，存量行默认 MANUAL。
- **发布动作登记**：工作流 publish 转换（总体结论写入十二章、`applyPublish` 之后）同步写一条 `kind=PUBLISH` 的版本记录——版本号由发布人在发布表单**手填**（不自动编号，校验规则与手动发版完全一致：必填、低于最新拒绝、重号拒绝、等于最新即覆盖），变更内容=总体结论（超 1000 字截断），修订人=发布人，快照=含结论的发布正文，所处阶段=PUBLISH。
- **发布快照存储不动**：`plan_publish_snapshots`（文档+场景+执行摘要三份 JSON）继续由发布动作写入，支撑分享链接与报告基线——版本表不承载场景/摘要负载。
- **发布 Tab 收缩**：移除「发布快照历史」区块（历史由版本 Tab 统一展示，发布来源条目带「报告发布」徽章），保留发布表单（结论+版本号）与分享链接管理。不回填历史发布快照为版本记录。
- 前端 `PlanVersionView` 增 `kind` 字段；版本列表按 kind 渲染徽章。
