...
- "验收标准"属 P0-3：已定**不建判等实体**——保存链严格校验指标表、报告生成时从原文确定性解析（2026-09-07 P0-3 设计 §3），P0-1 不建结构化列。
...
- P0-3 起（2026-09-07 设计定稿）：有指标计划达成表由判等引擎幂等重绘（`<!-- backfill:verdict -->` 块：数值实际列 + 状态 + 自动判定行）；总体结论不预写文档、发布弹窗预填自动判定文本，发布前人工确认（D4 半自动）；无指标计划（摸底/排查型）判等跳过、维持 P0-1 现状。
...
| P0-3 验收判等 | 指标清单以 Markdown 表格存于原文（书写面与判等输入）；P0-3 已定**不建判等实体**——保存链严格校验、报告生成时从原文确定性解析（2026-09-07 设计 §3）；生成报告时幂等重绘结论章节达成表并产出自动判定，总体结论发布时人工确认 |
