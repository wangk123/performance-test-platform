...
- **文档 Tab**（核心）：
  - **视图切换（分段控件 `Pretty | Markdown`，非 Tab）**：
    - `Pretty`：整份文档按章节顺序**连续展示**——测试目的与核心指标清单（指标表格表单）/ 测试范围（交易清单）/ 测试资源（人员/部署信息表）/ 测试约束（入口·出口准则清单）/ 场景设计（业务化场景 + 场景设置档位表）/ 叙述章节（Markdown 渲染）/ 执行记录（回填区块）/ 结论（达成表 + 风险建议 + 总体结论）。模块章节以受约束表单呈现（格式约束、快速编辑），**与叙述章节在同一文档流内原位排列，不重复堆叠**。**不含环境检查**（执行设置，见下）。
