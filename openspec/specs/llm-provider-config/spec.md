...
平台级 LLM 提供商配置：双 BaseUrl、密钥脱敏、拉列表/导入、连通性测试与级联删除。
...
- **THEN** each new model SHALL belong to that provider with the given apiType
- **AND** an already existing modelName under the same provider SHALL merge the apiType into its supported apiTypes
