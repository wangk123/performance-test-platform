...
统一 LLM 调用网关、协议适配器与调用记录落库查询。

## Requirements

### Requirement: Unified LLM gateway
The system SHALL expose a single LlmGateway invoke path that resolves model and provider, selects an adapter by resolved apiType (request override or model-supported default OPENAI), and writes a call record for both success and failure.

#### Scenario: Invoke OPENAI model
- **WHEN** a caller invokes the gateway with an enabled model that supports OPENAI (or requested apiType OPENAI)
...
- **WHEN** a caller invokes the gateway with an enabled model that supports ANTHROPIC (or requested apiType ANTHROPIC)
