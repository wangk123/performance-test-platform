...
**Reason**: The synchronous session model couples configuration, capture, Diff, and template inference, and cannot provide reusable strategies, durable samples, progress, or paged large-data access.

**Migration**: Remove legacy `seed_capture_session` data and use reusable capture strategies, independent snapshot samples, and manually started multi-sample analyses. The old start/sample/finish API flow is not retained.

#### Scenario: Legacy immediate capture flow is retired
- **WHEN** the redesigned capture workflow is deployed
- **THEN** creating or saving capture configuration does not invoke the legacy synchronous capture session flow
