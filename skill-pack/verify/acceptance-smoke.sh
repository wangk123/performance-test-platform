...
# 内经 SSE/JSON 双重编码后，键名在响应体中呈转义形式——断言负载键 \"items\"（项目列表
# 形状信号，强于对任何成功调用恒存在的封套键 \"data\"）。
if echo "$CALL" | grep -q '\\"items\\"'; then ok "结果含 items 列表负载"; else fail "结果缺少 items 负载: $CALL"; fi
