...
/**
 * trace 深度证据源启用后的端到端用例（T3）：SkyWalking OAP 不可达时必须显式
 * SOURCE_UNAVAILABLE 且 summary 携带错误信息，不得伪造空成功。
 */
@SpringBootTest(properties = {
        "spring.datasource.url=jdbc:h2:mem:agent-trace-source-test;MODE=MySQL;DATABASE_TO_LOWER=TRUE",
...
@Transactional
class AgentEvidenceTraceSourceApiTest {
    @Autowired
    ...
    void setUp() throws Exception {
        ...
        Path tempDir = Files.createTempDirectory("agent-trace-source-test");
    ...
    @Test
    void enabledTraceReportsSourceUnavailableWhenSkyWalkingUnreachable() throws Exception {
        ...
        assertThat(trace.get("sourceRef").asText()).startsWith("skywalking:trace");
        assertThat(trace.get("sourceClock").asText()).isEqualTo("target");
        assertThat(trace.get("summary").get("requiresApproval").asBoolean()).isTrue();
        assertThat(trace.get("summary").get("error").asText()).isNotBlank();
    }
